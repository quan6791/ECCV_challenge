...
fp = sorted(glob('../data_set/validation_output_09/*.jpg'))
model = Model()

load_npz('../model_step10.npz', model)

if device >= 0:
    model.to_gpu(device)
try:
    makedirs('../data_set/validation_output_10')
...
#     print x.shape
    ...
    y_hat.save('../data_set/validation_output_10/{}'.format(split(fp[i])[1])) # *contents* of the validation_output directory
