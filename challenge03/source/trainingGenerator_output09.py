...
fp = sorted(glob('../data_set/training_output_08/*.jpg'))
model = Model()
...
if device >= 0:
    model.to_gpu(device)
try:
    makedirs('../data_set/training_output_09')
...
#     print x.shape
    ...
    y_hat.save('../data_set/training_output_09/{}'.format(split(fp[i])[1])) # *contents* of the validation_output directory
